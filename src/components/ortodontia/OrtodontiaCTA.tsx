...
export const OrtodontiaCTA = () => {
    return (
        <EditableSectionColors sectionId="ortodontiaCTA" colors={CTA_COLORS}>
            {(colors) => (
                <section className="py-14 md:py-24 relative overflow-hidden" style={{ backgroundColor: colors.bg }}>
                    ...
                    <div className="container mx-auto px-4 md:px-6 relative z-10 text-center">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto flex flex-col items-center">
                            <EditableElement id="orto_cta_title_el" label="Título">
                                <h2 className="text-2xl md:text-[30px] font-serif uppercase tracking-wider mb-4 md:mb-6 leading-tight" style={{ color: colors.title }}>
                                    <EditableText id="orto_cta_title" defaultText="Seu sorriso pode ser alinhado com segurança e planejamento" />
                                </h2>
                            </EditableElement>
                            <EditableElement id="orto_cta_desc_el" label="Descrição">
                                <div style={{ color: colors.text }}>
                                    <EditableText id="orto_cta_desc" as="p" defaultText="Se você deseja alinhar seus dentes, melhorar sua mordida e cuidar da saúde bucal com acompanhamento especializado, agende sua avaliação na Oral Unic. Nossa equipe está pronta para indicar o melhor tratamento para o seu caso." className="text-sm mb-8 md:mb-12 font-light max-w-2xl mx-auto block" />
                                </div>
                            </EditableElement>
                            <EditableElement id="orto_cta_btn_el" label="Botão CTA">
                                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-white text-sm md:text-base px-8 md:px-10 py-3 md:py-4 rounded-full font-semibold hover:opacity-90 transition-all shadow-lg" style={{ backgroundColor: colors.btnBg }}>
                                    ...
                                    <EditableText id="orto_cta_btn" defaultText="Fale com um atendente" />
                                </a>
                            </EditableElement>
                        </motion.div>
                    </div>
