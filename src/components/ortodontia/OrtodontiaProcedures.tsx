...
const procedures = [
    { title: "Aparelho fixo metálico convencional", image: "/assets/ortodontia/proc_aparelho_fixo.jpg", delay: 0 },
    { title: "Alinhadores invisíveis removíveis", image: "/assets/ortodontia/proc_alinhadores.jpg", delay: 0.06 },
    { title: "Aparelho autoligado de alta tecnologia", image: "/assets/ortodontia/proc_autoligado.jpg", delay: 0.12 },
    { title: "Contenção e acompanhamento pós-tratamento", image: "/assets/ortodontia/proc_contencao.jpg", delay: 0.18 },
];

export const OrtodontiaProcedures = () => {
    return (
        <EditableSectionColors sectionId="ortodontiaProc" colors={PROC_COLORS}>
            {(colors) => (
                <section className="py-14 md:py-24 relative overflow-hidden" style={{ backgroundColor: colors.bg }}>
                    ...
                    <div className="container mx-auto px-4 md:px-6 relative z-10">
                        <EditableElement id="orto_proc_header_el" label="Cabeçalho">
                            <div className="text-center max-w-4xl mx-auto mb-10 md:mb-16 px-4">
                                <h2 className="text-2xl md:text-[30px] font-serif leading-tight italic" style={{ color: colors.title }}>
                                    <EditableText id="orto_proc_header" defaultText="Após a avaliação clínica, o tratamento pode envolver diferentes abordagens, como:" />
                                </h2>
                            </div>
                        </EditableElement>
                        ...
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
                            {procedures.map((proc, index) => (
                                <div key={index}>
                                    <EditableElement id={`orto_proc_card_${index}_el`} label={proc.title.substring(0, 20)}>
                                        <motion.div
                                            initial={{ opacity: 0, y: 30 }}
                                            whileInView={{ opacity: 1, y: 0 }}
                                            viewport={{ once: true }}
                                            transition={{ delay: proc.delay, duration: 0.5 }}
                                            className="group relative rounded-[1rem] md:rounded-[1.5rem] overflow-hidden shadow-lg aspect-[2/3] md:aspect-[3/5]"
                                        >
                                            <EditableImage
                                                id={`orto_proc_img_${index}`}
                                            ...
                                            <div className="absolute inset-0 flex items-end justify-center p-4 md:p-6 pointer-events-none">
                                                <h3 className="text-sm md:text-base font-serif font-bold leading-snug text-center select-all" style={{ color: colors.cardTitle }}>
                                                    <EditableText id={`orto_proc_title_${index}`} defaultText={proc.title} />
                                                </h3>
                                            </div>
                                        </motion.div>
                                    </EditableElement>
