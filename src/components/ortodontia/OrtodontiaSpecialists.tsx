...
const specialists = [
    {
        name: "Dra. Ydma Illeana",
        specialty: "Especialista em Ortodontia",
        description: "Atua com tratamentos ortodônticos modernos, incluindo aparelhos fixos, autoligados, alinhadores e técnicas auxiliares para correção da mordida.",
        image: "/assets/ortodontia/dra_ydma.jpg",
        instagram: "https://www.instagram.com/oralunicfsa",
    },
];

export const OrtodontiaSpecialists = () => {
    return (
        <EditableSectionColors sectionId="ortodontiaSpecialists" colors={SPEC_COLORS}>
            {(colors) => (
                <section className="py-14 md:py-24 relative overflow-hidden" style={{ backgroundColor: colors.bg }}>
                    <div className="container mx-auto px-4 md:px-6">
                        ...
                        >
                            <EditableElement id="orto_spec_title_el" label="Título">
                                <h2 className="text-2xl md:text-[30px] font-serif mb-4 md:mb-6 leading-tight">
                                    <span style={{ color: colors.titleAccent }} className="italic"><EditableText id="orto_spec_title" defaultText="Conheça nossos profissionais" /></span>
                                </h2>
                            </EditableElement>
                            <EditableElement id="orto_spec_subtitle_el" label="Subtítulo">
                                <div style={{ color: colors.text }}>
                                    <EditableText id="orto_spec_subtitle" as="p" defaultText="O tratamento ortodôntico na Oral Unic é conduzido por profissional especializada, com formação técnica e experiência clínica." className="text-sm max-w-3xl mx-auto block" />
                                </div>
                            </EditableElement>
                        </motion.div>
                                ...
                                >
                                    <div className="flex flex-row h-full">
                                        ...
                                        <div className="flex-1 p-5 md:p-6 flex flex-col justify-center min-h-[260px]">
                                            <EditableElement id={`orto_spec_info_${index}_el`} label={doc.name}>
                                                <h3 className="text-base md:text-lg font-serif font-bold text-white mb-1 leading-snug">
                                                    <EditableText id={`orto_spec_name_${index}`} defaultText={doc.name} />
                                                </h3>
                                                <p className="text-[11px] md:text-xs font-semibold mb-4 tracking-wide" style={{ color: colors.cardAccent }}>
                                                    <EditableText id={`orto_spec_specialty_${index}`} defaultText={doc.specialty} />
                                                </p>
                                                <p className="text-xs md:text-sm text-white/65 leading-[1.7] mb-5 font-light">
                                                    <EditableText id={`orto_spec_desc_${index}`} defaultText={doc.description} />
                                                </p>
                                                <a href={doc.instagram} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full border border-white/20 text-white text-xs font-medium hover:bg-white/10 transition-all w-fit">
                                                    ...
                                                    <EditableText id={`orto_spec_ig_${index}`} defaultText="Siga o Perfil" />
                                                </a>
                                            </EditableElement>
                                        </div>
                                    </div>
