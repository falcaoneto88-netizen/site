...
export const HarmonizacaoTestimonials = () => {
    return (
        <EditableSectionColors sectionId="harmoTestimonials" colors={TEST_COLORS}>
            {(colors) => (
                <section className="py-14 md:py-24 relative overflow-hidden" style={{ backgroundColor: colors.bg }}>
                    ...
                    <div className="container mx-auto px-4 md:px-6 relative z-10">
                        <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-12 md:mb-16 gap-6">
                            <div>
                                <EditableElement id="harmo_test_tag_el" label="Tag">
                                    <EditableText id="harmo_test_tag" defaultText="AVALIAÇÕES DO GOOGLE" className="text-[9px] md:text-[10px] font-bold tracking-[0.4em] uppercase text-primary/50 mb-3 block" />
                                </EditableElement>
                                <EditableElement id="harmo_test_title_el" label="Título">
                                    <h2 className="text-2xl md:text-[30px] font-serif leading-tight">
                                        <span style={{ color: colors.titleAccent }} className="italic"><EditableText id="harmo_test_title" defaultText="O que nossos pacientes dizem" /></span>
                                    </h2>
                                </EditableElement>
                                <div className="flex items-center gap-3 mt-4">
                                    <GoogleIcon />
                                    <div className="flex items-center gap-1.5">
                                        <span className="text-lg font-bold text-foreground"><EditableText id="harmo_test_rating" defaultText="4.9" /></span>
                                        <div className="flex gap-0.5">{[...Array(5)].map((_, i) => (<Star key={i} className="w-3.5 h-3.5 fill-amber-400 text-amber-400" />))}</div>
                                        <span className="text-xs text-muted-foreground ml-1"><EditableText id="harmo_test_count" defaultText="+500 avaliações" /></span>
                                    </div>
                                </div>
                            </div>
                            <EditableElement id="harmo_test_cta_el" label="Botão">
                                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="btn-primary group shrink-0" style={{ backgroundColor: colors.btnBg }}><EditableText id="harmo_test_cta" defaultText="Agende uma Consulta" /></a>
                            </EditableElement>
                        </div>
                            ...
                            {testimonials.map((item, index) => (
                                <motion.div key={index} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.12, duration: 0.6, ease: 'easeOut' }} className="group relative rounded-2xl md:rounded-3xl p-6 md:p-8 backdrop-blur-xl border transition-all duration-500 hover:shadow-2xl hover:-translate-y-1" style={{ backgroundColor: colors.glassBg, borderColor: colors.glassBorder, boxShadow: '0 4px 30px rgba(84,2,71,0.04)' }}>
                                    ...
                                    <div className="flex items-start justify-between mb-4 relative z-10">
                                        <div>
                                            <EditableText id={`harmo_test_name_${index}`} defaultText={item.name} className="font-bold text-foreground text-sm md:text-base block" />
                                            <EditableText id={`harmo_test_date_${index}`} defaultText={item.date} className="text-[10px] text-muted-foreground" />
                                        </div>
                                    ...
                                    <EditableText id={`harmo_test_text_${index}`} as="p" defaultText={item.text} className="text-xs md:text-sm text-muted-foreground leading-relaxed block relative z-10" />
                                </motion.div>
                            ))}
                        </div>
                        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.5, duration: 0.5 }} className="flex justify-center mt-10 md:mt-14">
                            <a href="https://search.google.com/local/reviews?placeid=ChIJExample" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-primary transition-colors group">
                                <GoogleIcon /><span><EditableText id="harmo_test_google_link" defaultText="Ver todas as avaliações no Google" /></span><ExternalLink className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
                            </a>
                        </motion.div>
                    </div>
                </section>
