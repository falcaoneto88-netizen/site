...
const procedures = [
    { title: "Correção de manchas", image: "/assets/facetas/proc_manchas.jpg", delay: 0 },
    { title: "Fechamento de diastemas", image: "/assets/facetas/proc_diastema.jpg", delay: 0.06 },
    { title: "Reparação de fraturas", image: "/assets/facetas/proc_fraturas.jpg", delay: 0.12 },
    { title: "Correção de formato", image: "/assets/facetas/proc_formato.jpg", delay: 0.18 },
    { title: "Harmonização do sorriso", image: "/assets/facetas/proc_harmonia.jpg", delay: 0.24 },
];

export const FacetasProcedures = () => {
    return (
        <EditableSectionColors sectionId="facetasProc" colors={PROC_COLORS}>
            {(colors) => (
                <section className="py-14 md:py-24 relative overflow-hidden" style={{ backgroundColor: colors.bg }}>
                    ...
                    <div className="container mx-auto px-4 md:px-6 relative z-10">
                        <EditableElement id="facetas_proc_header_el" label="Cabeçalho">
                            <div className="text-center max-w-4xl mx-auto mb-10 md:mb-16 px-4">
                                <h2 className="text-2xl md:text-[30px] font-serif leading-tight italic" style={{ color: colors.title }}>
                                    <EditableText id="facetas_proc_header" defaultText="Após avaliação clínica e exames, o tratamento pode envolver diferentes abordagens, como:" />
                                </h2>
                            </div>
                        </EditableElement>
                            ...
                            {procedures.map((proc, index) => {
                                ...
                                return (
                                    <div key={index} className={isLastOdd ? 'col-span-2 lg:col-span-1' : ''}>
                                        <EditableElement id={`facetas_proc_card_${index}_el`} label={proc.title.substring(0, 20)}>
                                            <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: proc.delay, duration: 0.5 }} className={`group relative rounded-[1rem] md:rounded-[1.5rem] overflow-hidden shadow-lg ${isLastOdd ? 'aspect-[2/1] md:aspect-[3/5]' : 'aspect-[2/3] md:aspect-[3/5]'}`}>
                                                <EditableImage id={`facetas_proc_img_${index}`} defaultSrc={proc.image} alt={proc.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                                ...
                                                <div className="absolute inset-0 flex items-end justify-center p-4 md:p-6 pointer-events-none">
                                                    <h3 className="text-sm md:text-base font-serif font-bold leading-snug text-center select-all" style={{ color: colors.cardTitle }}><EditableText id={`facetas_proc_title_${index}`} defaultText={proc.title} /></h3>
                                                </div>
                                            </motion.div>
                                        </EditableElement>
                                    </div>
