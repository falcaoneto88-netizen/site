...
export const ImplantCTA = () => {
    return (
        <EditableSectionColors sectionId="implantCTA" colors={CTA_COLORS}>
            {(colors) => (
                <section className="py-14 md:py-24 relative overflow-hidden" style={{ backgroundColor: colors.bg }}>
                    ...
                    <div className="container mx-auto px-4 md:px-6 relative z-10 text-center">
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-4xl mx-auto flex flex-col items-center">
                            <EditableElement id="implantcta_title_el" label="Título">
                                <h2 className="text-2xl md:text-[30px] font-serif uppercase tracking-wider mb-4 md:mb-6 leading-tight" style={{ color: colors.title }}>
                                    <EditableText id="implant_cta_title" defaultText="Seu sorriso merece um tratamento feito com critério, técnica e cuidado" />
                                </h2>
                            </EditableElement>
                            <EditableElement id="implantcta_desc_el" label="Descrição">
                                <div style={{ color: colors.text }}>
                                    <EditableText id="implant_cta_desc" as="p" defaultText="Se você perdeu um ou mais dentes e busca uma solução segura e duradoura, agende sua avaliação na Oral Unic. Nossa equipe está pronta para analisar seu caso com atenção e indicar o melhor caminho para você." className="text-sm mb-8 md:mb-12 font-light max-w-2xl mx-auto block" />
                                </div>
                            </EditableElement>
                            <EditableElement id="implantcta_btn_el" label="Botão CTA">
                                <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-white text-sm md:text-base px-8 md:px-10 py-3 md:py-4 rounded-full font-semibold hover:opacity-90 transition-all shadow-lg" style={{ backgroundColor: colors.btnBg }}>
                                    ...
                                    <EditableText id="implant_cta_btn" defaultText="Agendar avaliação agora" />
                                </a>
                            </EditableElement>
                        </motion.div>
                    </div>
