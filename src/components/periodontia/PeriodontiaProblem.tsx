...
import { Droplets, ShieldAlert, Bone, HeartPulse } from 'lucide-react';
...
const consequences = [
    { icon: Droplets, title: 'Sangramento gengival', desc: 'Gengivas que sangram ao escovar ou usar fio dental podem indicar inflamação ativa.' },
    { icon: ShieldAlert, title: 'Retração gengival', desc: 'A gengiva pode retrair expondo a raiz do dente, aumentando a sensibilidade.' },
    { icon: Bone, title: 'Perda de suporte ósseo', desc: 'A periodontite avançada destrói o osso que sustenta os dentes, podendo levar à perda dentária.' },
    { icon: HeartPulse, title: 'Impacto sistêmico', desc: 'Doenças periodontais estão associadas a condições como diabetes e doenças cardiovasculares.' },
];

export const PeriodontiaProblem = () => {
    return (
        <EditableSectionColors sectionId="perioProblem" colors={PROBLEM_COLORS}>
            {(colors) => (
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="text-center mb-12 md:mb-14">
                            <div className="flex items-center justify-center gap-4 mb-8">
                                ...
                                <span className="text-[10px] md:text-xs font-sans font-medium tracking-[0.25em] uppercase" style={{ color: colors.accent }}>
                                    <EditableText id="perio_problem_badge" defaultText="Você sabia?" />
                                </span>
                                <div className="w-10 h-px" style={{ backgroundColor: `${colors.accent}60` }} />
                            </div>
                            <EditableElement id="perio_problem_headline_el" label="Título">
                                <h2 className="font-serif text-2xl md:text-[30px] font-light leading-[1.2] mb-4" style={{ color: colors.text }}>
                                    <EditableText id="perio_problem_headline" defaultText="Problemas gengivais evoluem" />{' '}
                                    <span style={{ color: colors.accent }}><EditableText id="perio_problem_headline2" defaultText="silenciosamente" /></span>
                                </h2>
                            </EditableElement>
                            <EditableElement id="perio_problem_text_el" label="Subtexto">
                                <p className="text-sm font-sans font-light leading-[1.8] tracking-wide max-w-2xl mx-auto" style={{ color: `${colors.text}88` }}>
                                    <EditableText id="perio_problem_main_text" defaultText="O acompanhamento com um especialista em periodontia permite diagnosticar precocemente doenças gengivais e realizar o tratamento adequado para restaurar a saúde das gengivas e preservar os dentes naturais." />
                                </p>
                            </EditableElement>
                        </motion.div>
                        {/* DESKTOP */}
                                        ...
                                        <motion.div key={i} initial={{ opacity: 0, y: isTop ? -30 : 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.15, ease: 'easeOut' }} className={`flex flex-col items-center ${isTop ? '' : 'mt-20'}`}>
                                            ...
                                            <div className={`group rounded-2xl p-4 transition-all duration-500 hover:-translate-y-1 w-full ${isTop ? 'order-1' : 'order-2'}`} style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', backdropFilter: 'blur(8px)' }}>
                                                ...
                                                <div className="flex items-baseline gap-2 mb-2">
                                                    ...
                                                    <h3 className="font-sans text-sm font-semibold tracking-wide" style={{ color: colors.text }}><EditableText id={`perio_problem_cons_${i}`} defaultText={item.title} /></h3>
                                                </div>
                                                <p className="text-xs font-sans font-light leading-[1.8] tracking-wide" style={{ color: `${colors.text}77` }}><EditableText id={`perio_problem_desc_${i}`} defaultText={item.desc} /></p>
                                            </div>
                                        </motion.div>
                                    );
                                })}
                                            ...
                                            <div className="group rounded-2xl p-5 transition-all duration-500" style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)' }}>
                                                <div className="flex items-start gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs font-sans font-light leading-[1.7] tracking-wide" style={{ color: `${colors.text}77` }}><EditableText id={`perio_problem_desc_${i}`} defaultText={item.desc} /></p>
                                                    </div>
                                                </div>
                                            </div>
                                        </motion.div>
