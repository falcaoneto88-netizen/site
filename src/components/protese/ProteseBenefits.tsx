...
export const ProteseBenefits = () => {
    return (
        <EditableSectionColors sectionId="proteseBenefits" colors={BEN_COLORS}>
            {(colors) => (
                    ...
                    <div className="md:hidden flex flex-col">
                        <div className="relative w-full aspect-square overflow-hidden">
                            <motion.div initial={{ opacity: 0, scale: 1.05 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1.2, ease: "easeOut" }} className="w-full h-full">
                                <EditableImage id="protese_benefits_img_mobile" defaultSrc="/assets/protese/benefits_smile.jpg" alt="Como funciona o tratamento" className="w-full h-full object-cover" />
                            </motion.div>
                        ...
                        <div className="px-6 pt-6 pb-10">
                            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, ease: "easeOut" }}>
                                ...
                                <h2 className="text-2xl md:text-[30px] font-serif font-medium leading-[1.1] mb-4">
                                    <span style={{ color: colors.titleAccent }} className="italic"><EditableText id="protese_benefits_title_1" defaultText="Como funciona " /></span>
                                    <span style={{ color: colors.title }}><EditableText id="protese_benefits_title_2" defaultText="o tratamento" /></span>
                                </h2>
                                <div style={{ color: colors.text }} className="space-y-3">
                                    <EditableText id="protese_benefits_p1" as="p" defaultText="O tratamento começa com uma avaliação detalhada, onde são analisados dentes, gengiva, mordida e exames de imagem." className="text-sm leading-relaxed font-light block" />
                                    <EditableText id="protese_benefits_p2" as="p" defaultText="Com base nessas informações, é elaborado um plano de reabilitação personalizado, explicando as etapas, prazos e cuidados necessários." className="text-sm leading-relaxed font-light block" />
                                    <EditableText id="protese_benefits_p3" as="p" defaultText="Durante o processo, cada ajuste é realizado com atenção aos detalhes, buscando conforto, adaptação correta e um resultado estético natural. O acompanhamento após a finalização é essencial para garantir estabilidade e satisfação a longo prazo." className="text-sm leading-relaxed font-light block" />
                                </div>
                            </motion.div>
                        </div>
                    </div>
                    <div className="hidden md:block min-h-[70vh]">
                        <div className="absolute inset-0 z-0">
                            <motion.div initial={{ opacity: 0, scale: 1.05 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 1.2, ease: "easeOut" }} className="relative w-full h-full">
                                <EditableImage id="protese_benefits_img" defaultSrc="/assets/protese/benefits_smile.jpg" alt="Como funciona o tratamento" className="absolute right-0 top-0 h-full w-[65%] lg:w-[60%] max-w-[1100px] object-cover object-[80%_center]" />
