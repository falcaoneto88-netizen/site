...
import { Zap, ShieldAlert, Bug, AlertTriangle } from 'lucide-react';
...
const consequences = [
    { icon: Zap, title: 'Dor intensa e persistente', desc: 'A inflamação da polpa dentária causa dor aguda que pode irradiar para o ouvido, cabeça e pescoço.' },
    { icon: ShieldAlert, title: 'Risco de perda do dente', desc: 'Sem tratamento, a infecção avança e pode comprometer o dente de forma irreversível, levando à extração.' },
    { icon: Bug, title: 'Infecção pode se espalhar', desc: 'Uma infecção dentária não tratada pode se disseminar para outras regiões, formando abscessos graves.' },
    { icon: AlertTriangle, title: 'Comprometimento da saúde geral', desc: 'Infecções bucais crônicas estão associadas a problemas cardíacos, respiratórios e sistêmicos.' },
];

export const EndodontiaProblem = () => {
    return (
        <EditableSectionColors sectionId="endodontiaProblem" colors={PROBLEM_COLORS}>
            {(colors) => (
                        ...
                        >
                            <div className="flex items-center justify-center gap-4 mb-8">
                                ...
                                <span className="text-[10px] md:text-xs font-sans font-medium tracking-[0.25em] uppercase" style={{ color: colors.accent }}>
                                    <EditableText id="endo_problem_badge" defaultText="Atenção" />
                                </span>
                                <div className="w-10 h-px" style={{ backgroundColor: `${colors.accent}60` }} />
                            </div>

                            <EditableElement id="endo_problem_headline_el" label="Título">
                                <h2 className="font-serif text-2xl md:text-[30px] font-light leading-[1.2] mb-4" style={{ color: colors.text }}>
                                    <EditableText id="endo_problem_headline" defaultText="A dor de dente" />{' '}
                                    <span style={{ color: colors.accent }}>
                                        <EditableText id="endo_problem_headline2" defaultText="não deve ser ignorada" />
                                    </span>
                                </h2>
                            </EditableElement>

                            <EditableElement id="endo_problem_text_el" label="Subtexto">
                                <p className="text-sm font-sans font-light leading-[1.8] tracking-wide max-w-2xl mx-auto" style={{ color: `${colors.text}88` }}>
                                    <EditableText id="endo_problem_main_text" defaultText="A dor de dente costuma ser um dos desconfortos mais intensos que uma pessoa pode sentir. Em muitos casos, ela está relacionada a inflamações ou infecções internas do dente, causadas por cáries profundas, fraturas ou traumas." />
                                </p>
                            </EditableElement>
                        </motion.div>
                                            ...
                                            <div className={`group rounded-2xl p-4 transition-all duration-500 hover:-translate-y-1 w-full ${isTop ? 'order-1' : 'order-2'}`} style={{ backgroundColor: 'rgba(255,255,255,0.04)', border: `1px solid rgba(255,255,255,0.07)`, backdropFilter: 'blur(8px)' }}>
                                                ...
                                                <div className="flex items-baseline gap-2 mb-2">
                                                    ...
                                                    <h3 className="font-sans text-sm font-semibold tracking-wide" style={{ color: colors.text }}>
                                                        <EditableText id={`endo_problem_cons_${i}`} defaultText={item.title} />
                                                    </h3>
                                                </div>
                                                <p className="text-xs font-sans font-light leading-[1.8] tracking-wide" style={{ color: `${colors.text}77` }}>
                                                    <EditableText id={`endo_problem_desc_${i}`} defaultText={item.desc} />
                                                </p>
                                            </div>
                                        </motion.div>
                                    );
